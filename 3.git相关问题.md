...
`git show` 查看最后一次commit的详细改动

`git show --stat`查看改动了哪些文件
...
`git commit [dest file]`

可以在配置文件中修改使用什么编辑器编辑commit信息，可以使用vscode

`git config --global --edit`

editor使用code --wait，--wait是等待vscode关闭commit信息编辑页面之后再完成commit指令

![image-20260408173502833](https://raw.githubusercontent.com/upsetgrass/typora_pic_bed/main/image-20260408173502833.png)

# 上一次commit有遗漏并且push到远程分支A的快速补救

操作只会改变分支 A 的指针指向新的提交 ID。如果分支 B 是基于 A 的旧提交切出去的，或者有其他分支合并了 A，那么那些分支不会自动更新，甚至可能会产生冲突
...
git rm --cached xxx可以删除暂存区中的文件（不会把本地文件删除掉）

2、执行以下命令，这会将暂存区的文件直接合并进上一次的 commit 中，并允许你修改提交信息

`git commit --amend`

可以保持原有的 commit 信息不变（直接关闭窗口），也可以修改它，修改完成关闭commit编辑页面，
...
修改最近一次的提交（Commit），除了文件方面，commit信息也可以再次调整

--amend会修改hash值，所以如果已经push到远端，尽量不要用这个操作（除非该分支或确定无其他人pull下最新的代码）
...
git status发现与HEAD有差异

`git diff --cached` 全量查看

`git diff --cached -- file1` 查看某一文件的差异，这里的--代表再--后的是路径文件名，而不是分支名

`git diff --cached --stat` 查看每个文件修改了多少行	



--cached选项是针对已经在暂存区的，不加--cached就是还没放到暂存区的diff



ps：这个单独的双横杠 `--` 在许多 Linux 命令（不仅仅是 Git）中都有一个通用的含义：**“参数结束，后面全是路径（文件名）”。**



当你给 `git diff` 指定分支时，它会跳过暂存区，直接进行跨版本的对比

对比**两个远程/本地分支**之间的差异。

- *例子：* `git diff v5 main`。在处理 `Nanhu-V5-guanchuan` 时，如果你想看你的 `v5` 分支比官方 `main` 多了哪些特性，就用这个。



#### `git diff --cached <branch> -- <file>`

对比**暂存区**与**指定分支**中某个文件的差异。

- 如果你已经 `add` 了代码，想看看这些代码如果合并到 `main` 分支会产生什么冲突，可以用这个命令预览。
...
**看看你的暂存区和主分支 `main` 的 `SKILL.md` 有什么不同**：

```
git diff --cached main -- .github/skills/IFU/ifu-verify/SKILL.md
```
...
在 Git 中，`--cached`（在较新版本中也可以用 `--staged`）的含义是：**针对“暂存区（Index/Staging Area）”进行操作。**



**工作区 (Working Directory)**：你正在编辑的文件（还没 `add`）。

**暂存区 (Staging Area/Index)**：你执行 `git add` 后，准备提交的快照。

**版本库 (Repository/HEAD)**：已经 `commit` 的历史记录。



# --stat

在 Git 命令中，`--stat` 的作用是提供一个 **“数据摘要”**，而不是显示密密麻麻的每一行代码改动。