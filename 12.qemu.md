...
binfmt_misc机制：Linux内核有一个名为：Miscellaneous Binary Format的机制， 支持通过文件的特性（文件扩展名、文件开始位置特殊的字节（Magic Byte））来选择使用哪个程序来打开该文件
...
但是在/etc/fstab文件中没有这下面这行，再通过mount | grep binfmt_misc发现

![image-20250401145139407](https://raw.githubusercontent.com/upsetgrass/typora_pic_bed/main/image-20250401145139407.png)

说明binfmt_misc是自动挂载的，后面会详细讲解mount指令。
...
**挂载（mount）** 是将存储设备（如硬盘分区、光盘、USB 设备、网络共享等）连接到系统的文件系统中的过程。挂载后，用户可以通过文件路径访问存储设备上的数据，所有设备的文件最终都被组织到同一个目录树中。

**挂载**是一个非常重要的功能，使用非常频繁。它指将一个设备(通常是存储设备，可以挂载光盘、硬盘、磁带、光盘镜像文件等)**挂再到一个已存在的目录上，那么，你访问这个目录就是访问这个设备**（这个目录可以不为空，但挂载后这个目录下以前的内容将不可用，直到umount卸载后内容才能够重新可见）。
