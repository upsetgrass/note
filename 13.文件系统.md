...
（这里说的文件系统可以理解为一个目录树，比如 / 是一个文件系统， /home也是一个文件系统）

Linux维护了一个挂载表（/proc/mounts）记录当前所有挂载的文件系统，`cat /proc/mounts`或  `findmnt`即可查看挂载的内容

/etc/fstab用于定义系统启动时应该挂载的**持久性文件系统**（如根分区 `/`、EFI- `/boot/efi` 等），是静态文件系统，它不会列出运行时动态挂载的文件系统，比如 `tmpfs`、`sysfs`、`procfs`、`cgroup` 等。除了/etc/fstab，还有其他的例如：/run/mount  /etc/systemd/system/\*.mount  /etc/udev/rules.d/等会自动挂载
