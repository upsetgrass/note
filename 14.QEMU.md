...
- [QEMU文档](#qemu文档)
  ...
  - [基本使用](#基本使用)
    ...
    - [-m](#-m)
    - [-smp](#-smp)
    - [-hda和-drive](#-hda和-drive)
    - [-vga](#-vga)
    - [-plugin](#-plugin)
    - [-nographic](#-nographic)
  - [运行](#运行)
  - [-virtfs guest和host机共享文件](#-virtfs-guest和host机共享文件)
  - [qemu plugin](#qemu-plugin)
  - [kvm-qemu](#kvm-qemu)
- [分盘行为相关](#分盘行为相关)
...
禁用qemu图像界面，但不会自动将虚拟机的控制台输出重定向到您的终端。在这种情况下，虚拟机的输出会去到哪里，取决于虚拟机的操作系统配置。
