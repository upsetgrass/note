...
`qemu-system-x86_64 -m 4096 -smp 2 -drive file=disk.qcow2,format=qcow2 -cdrom ubuntu.iso -boot d   -vga virtio  –enable-kvm`  



```shell
qemu-system-x86_64 -m 4096 -M q35 -smp 2 \
  -drive file=disk_macos.qcow2,format=qcow2,if=virtio \
  -cdrom macOS_High_Sierra_10.13.6.iso \
...
qemu-system-x86_64 \
  -m 4096 \
  -cpu Penryn,vendor=GenuineIntel,kvm=on,+sse3,+sse4.2,+aes,+xsave,+avx,+xsaveopt \
  -machine q35 \
  -smp 2,cores=2 \
